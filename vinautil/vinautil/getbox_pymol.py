...
def __init__(self):
    ...
    self.menuBar.addmenuitem('GetBox Plugin', 'command', 'GetBoxHelp', label='Advanced usage',
                             command=lambda s=self: GetBoxHelp())
    self.menuBar.addmenuitem('GetBox Plugin', 'command', 'AutoBox', label='Autodetect box',
                             command=lambda s=self: autobox())
    self.menuBar.addmenuitem('GetBox Plugin', 'command', 'GetBox', label='Get box from selection (sele) ',
                             command=lambda s=self: getbox())
    self.menuBar.addmenuitem('GetBox Plugin', 'command', 'Remove HETATM', label='Remove HETATM ',
                             command=lambda s=self: rmhet())
...
def showbox(minX, maxX, minY, maxY, minZ, maxZ):
    ...
    CenterY = (maxY + minY) / 2
    CenterZ = (maxZ + minZ) / 2
    BoxCode = "BoxCode(" + boxName + ") = showbox %0.1f, %0.1f, %0.1f, %0.1f, %0.1f, %0.1f" % (
...
def getvinabox(selection="(sele)", extending=5.0) -> dict:
    # 获取 sele 对象的autodock vina盒子大小
    cmd.hide("spheres")
    cmd.show("spheres", selection)
    ([minX, minY, minZ], [maxX, maxY, maxZ]) = cmd.get_extent(selection)
    minX = minX - float(extending)
    ...
    CenterY = (maxY + minY) / 2
    CenterZ = (maxZ + minZ) / 2
    return {'cx': "{:.2f}".format(CenterX),
            'cy': '{:.2f}'.format(CenterY),
            'cz': "{:.2f}".format(CenterZ),
            'sx': "{:.2f}".format(SizeX),
            'sy': "{:.2f}".format(SizeY),
            'sz': "{:.2f}".format(SizeZ)}


def from_complex_get_box(ligandfile: str, chain: str, ligand_name: str, extending: int = 5) -> dict:
    # 从复合物中获取小分子的盒子大小 Get molecule box size from complex
    cmd.reinitialize('everything')  # ! clean up
    cmd.load(ligandfile)  # load file
    cmd.select('obj_select', f'( resn {ligand_name} ) and chain {chain}')  # select ligand as object named sele
    box = getvinabox(selection="(obj_select)", extending=extending)
